...
namespace tsundoku
{
  ...
  static const std::vector<uint32_t> plane_indices = {
    0, 1, 2,
    2, 3, 0,
  };
  ...
  RenderSystem::RenderSystem(Platform &platform, Camera &camera)
  {
    ...
    m_vertex_buffer  = std::make_unique<VertexBuffer>(*m_context);
    ...
    m_command_buffer = std::make_unique<CommandBuffer>(*m_context);
    ...
    m_vertex_buffer->upload_vertices(plane_vertices);
    m_vertex_buffer->upload_indices(plane_indices);
  }
  ...
  void RenderSystem::draw_frame()
  {
    ...
    m_command_buffer->reset(m_current_frame);
    ...
    VkCommandBuffer      cmd                 = m_command_buffer->get(m_current_frame);
  ...
  void RenderSystem::record(uint32_t image_index)
  {
    m_command_buffer->begin(m_current_frame);
    ...
    m_vertex_buffer->bind_vertex(cmd);
    m_vertex_buffer->bind_index(cmd);

    VkDescriptorSet desc_set = m_camera_vulkan->get_descriptor_set(m_current_frame);
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
        m_pipeline->get_layout(), 0, 1,
        &desc_set, 0, nullptr);
    ...
    vkCmdDrawIndexed(cmd, m_vertex_buffer->get_index_count(), 1, 0, 0, 0);

    vkCmdEndRenderPass(cmd);
    m_command_buffer->end(m_current_frame);
  }
